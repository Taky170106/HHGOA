...
export function SiteNav() {
  ...
  return (
    <header className="z-30 shrink-0 border-b border-line bg-ink-950/95 backdrop-blur">
      <div className="flex h-14 items-center gap-5 overflow-x-auto px-5">
        {/* identity */}
        <Link href="/" className="flex shrink-0 items-center gap-2.5">
          <span className="flex h-8 w-8 items-center justify-center rounded-sm border border-accent/50 bg-accent/10 font-mono text-[14px] text-accent">
            ⬡
          </span>
          <span className="leading-none">
            <span className="block text-[13px] font-bold tracking-[0.18em] text-fg">
              HHG <span className="text-accent">//</span> FRAUD INTELLIGENCE
            </span>
            <span className="mt-[3px] block text-[8.5px] tracking-[0.22em] text-dim">
          ...
          {NAV.map((n) => {
            ...
            return (
              <Link
                key={n.href}
                href={n.href}
                className={`relative px-3 py-2 text-[11px] font-bold tracking-[0.12em] transition ${
                  active ? "text-accent" : "text-fg-2 hover:text-fg"
                }`}
              >
                {n.label}
                <span
                  className={`absolute inset-x-2 -bottom-[1px] h-[2px] transition ${
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          ...
          <Link
            href="/investigation"
            className="inline-flex h-8 items-center gap-1.5 rounded-sm border border-accent/60 bg-accent/15 px-3 text-[10.5px] font-bold tracking-[0.1em] text-accent transition hover:bg-accent/25"
          >
            ▶ RUN INVESTIGATION
          </Link>
        </div>
...
export function StatusFooter() {
  ...
  return (
    <footer className="z-30 flex h-7 shrink-0 items-center gap-5 overflow-x-auto border-t border-line bg-ink-950 px-5 font-mono text-[9.5px] whitespace-nowrap text-dim">
      <span>
        VALIDATION{" "}
